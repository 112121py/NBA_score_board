...
public class crawl {
	...
	public void main() throws IOException{
		Document doc = Jsoup.connect("https://www.espn.com/nba/scoreboard/_/date/" + date).get();
        nbatitle = doc.title();
            ...
            if (text.matches(".*\\d+.*") || namecheck(nbateam,text) &&text!="" ) {
            	...
            	if (thisIsScore.contains(cnt) ||cnt==1) {
            		...
            		if (gamecnt==1) {
            			scorerec[1]=score;
            			System.out.println();
            			temp.add(scorerec[0]+" : "+scorerec[1]);
            			...
            			if (scorerec[0]>scorerec[1]) {
            				temp.add(namerec[0]+" wins");
            				System.out.println(namerec[0]+" wins");
            			}
            			else {
            				temp.add(namerec[1]+" wins");
